...
import { Building2, Plus, Search, Eye, CheckCircle, ShieldAlert } from 'lucide-react';
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Directory of clients, building accounts, and customer login statuses.</p>
        </div>
        <Link
          href="/customers/new"
            ...
            <tbody className="divide-y divide-slate-100">
              {customers.map((c) => (
                <tr key={c._id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="p-4">
                    {c.hasLogin ? (
                      <span className="px-2.5 py-1 rounded-full text-xs font-semibold bg-emerald-50 text-emerald-700 border border-emerald-200 inline-flex items-center space-x-1">
