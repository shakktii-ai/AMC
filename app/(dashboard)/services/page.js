...
export default function ServicesPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Services & Preventive Maintenance</h1>
        <p className="text-sm text-slate-500 mt-0.5">Service visit schedule, PPM routines, and technician assignments.</p>
      </div>

      {loading ? (
        <LoadingSpinner message="Fetching service schedules..." />
      ) : (
          ...
          <table className="w-full text-left text-sm text-slate-700">
            <thead className="bg-slate-50 border-b border-slate-200 text-slate-500 uppercase text-[11px] font-bold tracking-wider">
              <tr>
                ...
                <th className="p-4">Customer & Lift</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {services.map((s) => (
                <tr key={s._id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="p-4">
                    <div className="font-bold text-slate-900">{s.customerId?.name}</div>
                    <div className="text-xs text-slate-500">{s.liftId?.buildingName} ({s.liftId?.assetCode})</div>
                  </td>
                  ...
                  <td className="p-4">
                    <Badge variant={s.status === 'COMPLETED' ? 'success' : s.status === 'IN_PROGRESS' ? 'warning' : 'default'}>
                      {s.status}
                    </Badge>
                  </td>
