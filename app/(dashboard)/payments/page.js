...
export default function PaymentsPage() {
  ...
  const canRecordPayment = ['SUPER_ADMIN', 'ADMIN', 'ACCOUNTANT'].includes(userRole);
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-8 text-center text-slate-500 space-y-3">
          ...
          <p className="text-xs max-w-sm mx-auto">
            {userRole === 'CUSTOMER'
              ? 'No payment transactions recorded for your account invoices.'
              : 'No payment records available. Record payments against issued invoices.'}
          </p>
        </div>
      ) : (
